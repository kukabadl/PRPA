...
unsigned char input (){
  ...
  //pMat0 = *matrix0;
  //printf("matrix0[0] = %d", *(pMat0+6));
  //for (int i = 0; i < sizeof (matrix0)/4; i++){

  pMat0 = (int*)calloc(rozm[iter][0]*rozm[iter][1], sizeof(int));
  for (int i = 0; i < sizeof (*pMat0); i++){
    if (1 != scanf ("%d", &in)){
      fprintf(stderr, "Error: Chybny vstup!\n");
      return 100;
	   }
  ...
  //pMat1 = *matrix1;
  pMat1 = (int*)calloc(rozm[iter][0]*rozm[iter][1], sizeof(int));
  //for (int i = 0; i < sizeof (matrix1)/4; i++){
  for (int i = 0; i < sizeof (*pMat1); i++){
    if (1 != scanf ("%d", &in)) {
      fprintf(stderr, "Error: Chybny vstup!\n");
      return 100;
    }
